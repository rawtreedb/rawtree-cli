...
pub fn databases_collection_path(organization: Option<&str>) -> String {
    match organization {
        Some(org_name) => format!(
            "/v1/databases?organization={}",
            urlencoding::encode(org_name)
        ),
        None => "/v1/databases".to_string(),
    }
}

pub fn database_scoped_path(database: &str, suffix: &str, organization: Option<&str>) -> String {
...
mod tests {
    ...
    fn databases_collection_path_uses_organization_filter() {
        assert_eq!(
            databases_collection_path(Some("team alpha")),
            "/v1/databases?organization=team%20alpha"
        );
        assert_eq!(databases_collection_path(None), "/v1/databases");
    }

    #[test]
    fn database_scoped_path_adds_database_query_param() {
        let path = database_scoped_path("analytics", "/query", None);
        assert_eq!(path, "/v1/query?database=analytics");
    }

    #[test]
    fn database_scoped_path_adds_organization_query_param() {
        let path = database_scoped_path("analytics", "/query", Some("team_alpha"));
    ...
    fn database_scoped_path_accepts_suffix_without_leading_slash() {
        let path = database_scoped_path("analytics", "tables", Some("team_alpha"));
        assert_eq!(
            path,
    ...
    fn database_scoped_path_appends_to_existing_query() {
        let path = database_scoped_path("analytics db", "/tables/events?url=x", Some("team a"));
        assert_eq!(
            path,
